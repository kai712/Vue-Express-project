...
<script>
  import { getMaterielClassify } from '../../../api/materiel'
  import { post } from '../../../api'
  export default {
    name: 'materielClassification',
    data() {
      return {
        data: [],
        defaultProps: {
    ...
    methods: {
      ...
      confirmForm(){
        ...
        post('/api/editMaterielClassify',item).then((res) => {
          if( res.result === 1 ) {
            this.$message({
              message: res.message,
              type: 'success',
      ...
      remove(store, data) {
        ...
        if(data.children){
          ...
          this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
            ...
            post('/api/delMaterielClassify', {_id: id}).then((res) => {
              if( res.result === 1 ) {
